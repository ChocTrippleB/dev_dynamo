...
// No need for frontend API key now – backend talks to OpenAI
// const API_KEY = "" 
...
export default function Home() {
  ...
  // NEW: language state (used by the existing <select>, UI unchanged)
  ...
  // REPLACED: call backend instead of OpenAI directly
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] w-full bg-gray-900 text-white relative">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex justify-between items-center shadow-sm shrink-0 mb-4">
        ...
        <div className="flex space-x-2">
          {/* Wire select to language state (no UI change) */}
          <select
            className="text-sm border-gray-600 border rounded-md px-2 py-1 bg-gray-700 text-white focus:ring-indigo-500 focus:border-indigo-500 outline-none shadow-sm cursor-pointer"
            value={language}
            onChange={(e) => setLanguage(e.target.value.toLowerCase())}
          >
            <option className="bg-gray-700 text-white" value="english">
              English
            </option>
            <option className="bg-gray-700 text-white" value="zulu">
              Zulu
            </option>
            <option className="bg-gray-700 text-white" value="xhosa">
              Xhosa
            </option>
          </select>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-6 w-full pb-32 px-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4 mt-[-50px]">
            ...
            <p className="text-lg font-medium text-gray-300">
              How can I help you pass today?
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-md">
              <button
                onClick={() =>
                  setInput("Explain Newton's Laws using a taxi example")
                }
                className="p-4 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-indigo-500 rounded-xl text-sm text-left shadow-sm transition-all text-gray-200 flex items-center group cursor-pointer"
              >
              ...
              <button
                onClick={() =>
                  setInput("Give me a quiz on Macbeth characters")
                }
                className="p-4 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-indigo-500 rounded-xl text-sm text-left shadow-sm transition-all text-gray-200 flex items-center group cursor-pointer"
              >
                <span className="mr-3 text-xl group-hover:scale-110 transition-transform">
        ...
        {messages.map((msg, idx) => (
          ...
          >
            ...
            >
              <div
                className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mx-2 shadow-sm ${
                  msg.role === "user"
              ...
              <div
                className={`p-4 rounded-2xl shadow-sm relative text-sm md:text-base leading-relaxed ${
                  msg.role === "user"
                    ? "bg-indigo-600 text-white rounded-tr-none"
                    : "bg-gray-800 border border-gray-700 text-gray-200 rounded-tl-none"
                }`}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-20 bg-gradient-to-t from-gray-900 via-gray-900 to-transparent">
        <div className="max-w-4xl mx-auto flex items-end space-x-2 bg-gray-700 p-2 rounded-3xl border border-gray-600 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:bg-gray-800 transition-all shadow-lg">
          <button
            onClick={toggleListening}
            className={`p-3 rounded-full transition-all flex-shrink-0 border-none shadow-none cursor-pointer ${
              isListening
                ? "bg-red-500 text-white animate-pulse"
                : "bg-transparent text-gray-400 hover:text-white hover:bg-gray-600/50"
            }`}
          >
            {isListening ? <MicOff size={20} /> : <Mic size={20} />}
          </button>
        ...
        <div className="text-center mt-2 text-xs text-gray-500">
          AI can make mistakes. Check with your textbook.
        </div>
      </div>
    </div>
  );
